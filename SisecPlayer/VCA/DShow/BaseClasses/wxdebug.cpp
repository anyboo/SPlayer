...
void WINAPI DbgBreakPoint(const TCHAR *pFileName,INT iLine,const TCHAR* szFormatString,...)
{
	...
	int nReturnValue = _stprintf_s( szBreakPointMessage, MAX_CHARS_IN_BREAK_POINT_MESSAGE, szFormatString, va );
...
void WINAPI DbgLogInfo(DWORD Type,DWORD Level,const TCHAR *pFormat,...)
{
    ...
    lstrcpy(szInfo,m_ModuleName);
    _stprintf_s(szInfo + lstrlen(szInfo), _countof(szInfo) - lstrlen(szInfo) - 1,
             TEXT("(tid %x) %8d : "),
             GetCurrentThreadId(), timeGetTime() - dwTimeOffset);

	_stprintf_s(szInfo + lstrlen(szInfo), _countof(szInfo) - lstrlen(szInfo) - 1, pFormat, va);
    lstrcat(szInfo, TEXT("\r\n"));
    DbgOutString(szInfo);
...
{
#ifdef DEBUG
	_stprintf_s(m_String, _countof(m_String), TEXT("%.16g"), d);
#else
